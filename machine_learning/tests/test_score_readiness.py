...
CONFIG_V6_PATH = (
    Path(__file__).resolve().parents[2] / "backend" / "config" / "framework.v6.json"
)
...
def _config_v6() -> dict[str, object]:
    return json.loads(CONFIG_V6_PATH.read_text(encoding="utf-8"))
...
def test_framework_v6_feature_global_uses_enabled_feature_support_ratio() -> None:
    config = _config_v6()
    support = {
        "users": 200,
        "observations": 10_000,
        "mean_feature_coverage": 0.50,
        "enabled_feature_support_ratio_at_200": 0.90,
    }
    common = {
        "branch": "feature",
        "level": "GLOBAL",
    ...
    assert _ready(entry={"support": support}, **common)
    assert not _ready(
        entry={
            "support": {
                **support,
                "mean_feature_coverage": 1.0,
                "enabled_feature_support_ratio_at_200": 0.899999,
            }
        },
        **common,
    )
...
def test_framework_v6_reference_contract_and_inactive_day_policy(
    tmp_path: Path,
) -> None:
    checkpoint = tmp_path / "checkpoint-v6.pt"
    checkpoint.write_bytes(b"immutable-checkpoint")
    config = _config_v6()
    artifact = _fit_reference_artifact(
        _role_ready_reference_rows(60),
        checkpoint_path=checkpoint,
        preprocessing_checksum="b" * 64,
        scaler_checksum="c" * 64,
        framework_config=config,
    ...
    assert artifact["schema_version"] == "hierarchical-reference.v3"
    assert artifact["entry_contract_version"] == "hierarchical-reference-entry.v2"
    global_support = artifact["branches"]["feature"]["GLOBAL"]["GLOBAL"][
        "support"
    ]
    assert global_support["enabled_feature_count"] == 124
    assert global_support["enabled_feature_support_ratio_at_40"] == 1.0
    _validate_reference_artifact_contract(
        artifact,
        framework_config=config,
        framework_config_checksum="d" * 64,
    ...
    assert active["feature_level"] == "PERSON"
    assert active["status"] == "SCORED"
    ...
    assert calibrated["status"] == "NO_SCORE"
    assert calibrated["risk"] is None
    assert calibrated["_readiness_trace"] == {
